import { Font, FontLoader, Group, Material, Mesh, MeshPhongMaterial, TextGeometry } from "three";
...
export default class MeshText {
    ...
    constructor(textVal: string, private onLoadComplete: (obj: MeshText) => void, settings?: MeshTextSettingsParam) {

        if (settings && settings.color && typeof settings.color === 'string') {
            if (settings.color.includes('0x')) {
                settings.color = Number(settings.color);
            }
        }

        this._settings = Tools.merge(this._settings, settings || {}) as MeshTextSettings;

        this._text = textVal;

        if (MeshText.reverseFontMap.length == 0)
            for (const i in MeshText.fontMap) MeshText.reverseFontMap[MeshText.fontMap[i]] = i;

        if (MeshText.reverseWeightMap.length == 0)
            for (const i in MeshText.weightMap) MeshText.reverseWeightMap[MeshText.weightMap[i]] = i;

        let matKey = this.settings.color.toString();

        if (!(matKey in MeshText.matCache)) {
            MeshText.matCache[matKey] = [
                new MeshPhongMaterial({ color: this.settings.color, flatShading: true, wireframe: false, reflectivity: 0, shininess: 0}), // front
                new MeshPhongMaterial({ color: 0x000000 }) // side
            ];
        }

        this.materials = MeshText.matCache[matKey];
    ...
    private static meshTextFontWeightToString(weight: MeshTextFontWeights): string {
        let weightString = "";
        let target = -1;
    ...
    private static fontSettingsToAsset(font: MeshTextFonts, weight: MeshTextFontWeights) {
        let asset;
    ...
    private loadFont(onLoadComplete: (obj: MeshText) => void) {
        let fontKey = MeshText.meshTextFontToString(this.settings.font) + "_" + MeshText.meshTextFontWeightToString(this.settings.fontWeight);

        if (!(fontKey in MeshText.fontCache)) {
            let asset = MeshText.fontSettingsToAsset(this.settings.font, this.settings.fontWeight);
    ...
    private createText(onCreateComplete: (obj: MeshText) => void) {
        ...
        let geoKey = this.text + "_" + fontKey + "_" + this.settings.size + "_" + this.settings.height + "_" + this.settings.curveSegments + "_" + this.settings.bevelEnabled + "_" + this.settings.bevelThickness + "_" + this.settings.bevelSize
        ...
        const centerOffsetY = - 0.5 * (this.textGeo.boundingBox.max.y - this.textGeo.boundingBox.min.y);
        this._textMesh1 = new Mesh(this.textGeo, this.materials);

        this.textMesh1.position.x = centerOffsetX;
        this.textMesh1.position.y = centerOffsetY + this.settings.hover;
        ...
        if (this.settings.mirror) {
            ...
            this.textMesh2.position.y = centerOffsetY - this.settings.hover;
            this.textMesh2.position.z = this.settings.height;
