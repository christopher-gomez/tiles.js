import { BoxBufferGeometry, BoxGeometry, BoxHelper, BufferAttribute, ClampToEdgeWrapping, Color, DoubleSide, Euler, Float32BufferAttribute, InstancedBufferAttribute, InstancedBufferGeometry, InstancedMesh, LinearMipMapLinearFilter, Material, Matrix4, Mesh, MeshNormalMaterial, MeshPhongMaterial, Object3D, PlaneBufferGeometry, PlaneGeometry, Quaternion, RawShaderMaterial, RepeatWrapping, Scene, ShaderMaterial, TextureLoader, Vector2, Vector3, } from "three";
import View from "../../../lib/scene/View";
import { createMountains, generateSnowflakeSystem as generateSnowflakes } from ".";
import mtnTAsset from '../../../Assets/Textures/alex-beholder-uqt_yf4f_ME-unsplash.jpg';
...
export default async (containers: Tile[], boundary: Vector2[], view: View) => {
    ...
    const t = await loader.loadAsync(mtnTAsset);

    console.log('done loading mtn t')

    t.wrapS = t.wrapT = ClampToEdgeWrapping;
    t.minFilter = LinearMipMapLinearFilter;
    t.anisotropy = view.renderer.capabilities.getMaxAnisotropy();
    const { ground, noise } = createMountains(width, height, resolution, boundary, containers[0], t);
    // Calculate the base rotation
    const baseRotation = new Euler(0, -Math.PI / 2, 0);
    // Create the instanced mesh
    const instanceCount = containers.length;
    let mat = ground.material;
    const groundInstances = new InstancedMesh(ground.geometry.clone(), mat, instanceCount);
    ...
    const flakes = [];
    for (let i = 0; i < instanceCount; i++) {
        const { system, render } = generateSnowflakes(width, height, boundary, containers[0]);
