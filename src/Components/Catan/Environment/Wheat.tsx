import { Box3, BufferGeometry, Euler, Frustum, Geometry, Group, InstancedMesh, LinearMipMapLinearFilter, Matrix4, Mesh, RepeatWrapping, TextureLoader, Vector2, Vector3, } from "three";
import View from "../../../lib/scene/View";
import { generateGrassSystem, createFarmland, dirtNoiseOpts } from ".";
import Tile from "../../../lib/map/Tile";
import farmGroundTexture from '../../../Assets/Textures/HITW-TS2-farm-browns.jpg';
...
export default async (containers: Tile[], boundary: Vector2[], view: View) => {
    ...
    const t = await loader.loadAsync(farmGroundTexture);
    ...
    // Populate the instance positions
    for (let i = 0; i < instanceCount; i++) {
        const grass = generateGrassSystem(width, height, boundary, containers[i], view.camera, view.controller, ground.geometry, containers[i].geometry.boundingBox.max.z + .1, 2, dirtNoiseOpts, 20000, false);
    ...
    view.animationManager.addOnAnimateListener(() => {
        ...
        //     const grass = grasses[i];
        //     const tilePositionInCameraSpace = containers[i].mesh.position.clone().applyMatrix4(rotationMatrix);
        //     // Check if the bounding box of the tile mesh intersects the frustum
