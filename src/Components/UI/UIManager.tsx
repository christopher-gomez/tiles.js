...
} from "./SceneSettings/CameraSettings";
import SmartToyIcon from "@mui/icons-material/SmartToy";
import EntitiesMenu, {
  IEntitiesMenuComponentProps,
} from "./SceneSettings/EntitiesMenu";
import DesignMapMenu, {
  IDesignMapMenuComponentProps,
} from "./SceneSettings/DesignMapMenu";
    ...
    <>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            disabled={!mapName || !isNaN(parseFloat(mapName))}
            onClick={() => {
              if (onSaveScene)
                onSaveScene(mapName, (JSON) => {
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            disabled={
              selectedLoadFile === null || selectedLoadFile === undefined
            }
            onClick={async () => {
      ...
      <Box
        sx={{
          ...
          pl: "0 !important",
          top: 0,
          left: !sideOpen ? theme.spacing(7) : 240,
        ...
        {curHighlightedTile && (
          ...
          >
            <Typography>
              Mesh Position:{" "}
              {"(" +
                curHighlightedTile.mapPosition
                  .toArray()
                  .map((x) => Math.round(x))
                  .toString()
                  .replaceAll(",", ", ") +
            ...
            <Typography>
              Cell Position:{" "}
              {"(" +
                curHighlightedTile.cell?.gridPosition
                  .toArray()
                  .map((x) => Math.round(x))
                  .toString()
                  .replaceAll(",", ", ") +
            ...
            <Typography>
              Custom Data:{" "}
              {JSON.stringify(curHighlightedTile.getCustomData(), null, 2)}
            </Typography>
          </Box>
        )}
