...
export interface SideBarProps {
    open: boolean,
    handleToggle: () => void
    buttonGroups: Array<Array<SidebarButton>>
}
                    ...
                    {group.filter(g => g !== undefined).map(button => (
                        <ListItem key={button.name} disablePadding sx={{ display: 'block' }}>
                            ...
                            >
                                ...
                                {button.subItems && open ?
                                    (
                                        button.rightIcon ?? <ListItemIcon>
                                            {button.subItemsOpen ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                                        </ListItemIcon>
                                    )
                                    : open && (button.rightIcon ?? <></>)
                                }
                            </ListItemButton>
                            {button.subItems && open &&
                                <>
                                    <Collapse in={button.subItemsOpen} timeout="auto" unmountOnExit>
                                        <List component="div" disablePadding>
                                            {button.subItems.map(innerButton => (
                                                innerButton
                                            ))}
                                        </List>
